...
def build_vocab_idx(word_insts, min_word_count):
    ...
    full_vocab = set(w for sent in word_insts for w in sent)# it's so good
    ...
    for word, count in word_count.items():# items() can get the info in the item 
...
def convert_instance_to_idx_seq(word_insts, word2idx):
    '''Word mapping to idx'''
    try:
        word_insts = ['</s>' if x is None else x for x in word_insts]#so good
