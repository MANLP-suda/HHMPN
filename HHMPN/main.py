...
def main(opt):
	...
	# global_labels = None
	# for i in range(len(data['train']['src-com'])):
	...
	# for i in range(len(data['valid']['src-com'])):
	# 	labels = torch.tensor(data['valid']['tgt'][i]).unsqueeze(0)
	# 	labels = utils.get_gold_binary_full(labels,vocab_size)
	# 	global_labels+=labels
		
	# for i in range(len(data['test']['src-com'])):
	...
	#========= Preparing Model =========#
	model = LAMP(
		# opt.src_com_vocab_size,
		# opt.src_lyr_vocab_size,
		opt.tgt_vocab_size,
		opt.max_token_text_seq_len_e,
		opt.max_token_visual_seq_len_e,
		opt.max_token_audio_seq_len_e,
