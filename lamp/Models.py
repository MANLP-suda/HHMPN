...
class LAMP(nn.Module):
    def __init__(
            self, n_tgt_vocab, n_max_text_seq_e,n_max_visual_seq_e,n_max_audio_seq_e,n_max_seq_d,text_dim,visual_dim,audio_dim, n_layers_enc=6,n_layers_dec=6,
        ...
        if encoder == 'mlp':
            self.encoder = MLPEncoder(
                n_src_com_vocab, n_max_com_seq_e,n_src_lyr_vocab,n_max_lyr_seq_e, n_layers=n_layers_enc, n_head=n_head,
                d_word_vec=d_word_vec, d_model=d_model,d_k=d_k, d_v=d_v,
                d_inner_hid=d_inner_hid,onehot=onehot, dropout=dropout)
        elif encoder == 'graph':
            self.encoder = GraphEncoder(
                n_max_text_seq_e,n_max_visual_seq_e,n_max_audio_seq_e,text_dim,visual_dim,audio_dim, n_layers=n_layers_enc, n_head=n_head,
                d_word_vec=d_word_vec, d_model=d_model,d_k=d_k, d_v=d_v,
                d_inner_hid=d_inner_hid, onehot=onehot, dropout=dropout,
        ...
        if decoder == 'rnn_m':
            ...
            self.decoder = MLPDecoder(
                n_tgt_vocab, n_max_com_seq_e,n_max_lyr_seq_e, n_max_seq_d, n_layers=n_layers_dec, n_head=n_head,
                d_word_vec=d_word_vec, d_model=d_model,d_k=d_k, d_v=d_v,
    ...
    def forward(self, src, adj, tgt_seq, binary_tgt,return_attns=False,int_preds=False):
        '''
        @description: 
        @param {src} [0]:[B,n_word]:denotes that the word id in vocab [1]:[B,n_word]:denote the range from 1 to the length of the sequence
        @parm{binary_tgt}: [B,L]:
        @return: 
        '''
        batch_size = src[0].size(0)
        src_text_seq, src_text_pos, src_visual_seq, src_visual_pos,src_audio_seq, src_audio_pos = src
        if self.decoder_type in ['sa_m','rnn_m']: 
            tgt_seq = tgt_seq[:, :-1]
        # stop()
        enc_text_output,enc_visual_output,enc_audio_output, *enc_self_attns = self.encoder(src_text_seq,src_visual_seq,src_audio_seq, adj, src_text_pos,src_visual_pos,src_audio_pos,return_attns=return_attns)
        dec_output, *dec_output2 = self.decoder(tgt_seq,src_text_seq,enc_text_output,src_visual_seq,enc_visual_output,src_audio_seq,enc_audio_output,return_attns=return_attns,int_preds=int_preds)
        # stop()
        ...
        if int_preds:
            ...
            return seq_logit.view(-1,seq_logit.size(-1)),enc_text_output,enc_visual_output,None
